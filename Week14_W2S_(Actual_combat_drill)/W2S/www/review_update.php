...
//--------------------------------------	   
	...
	$join_id=(isset ($_POST['join_id'])) ? $_POST['join_id'] : "";
	$status=(isset ($_POST['status'])) ? $_POST['status'] : "";
	$note=(isset ($_POST['note'])) ? $_POST['note'] : "";
	$review_id=$ck_user_no;
	$modify_date=date("Y-m-d H:i:s");	// comment_schedule
	$join_date2=date("Y-m-d H:i:s");	// comment_info
	$modify_date2="NULL";			// comment_info
//--------------------------------------
...
//--------------------------------------	
	...
	{	
		...
		if ($result)
		{
			$msg="<font color='blue'>"."維護評核行程-OK"."</font>";
		}
		else
		{		
			$msg="<font color='red'>"."維護評核行程-ERROR"."</font>";
		}	
		//--------------------------------------
		if ($status=="2")
		{
			$sql =" select count(*) from comment_info";
			$sql.=" where schedule_no='$schedule_no'";
			...
			{	
				...
				if ($result)
				{
					//$msg.="<font color='blue'>"."新增comment_info-OK"."</font>";			
				}
				else
				{		
					$msg.="<font color='red'>"."新增comment_info-ERROR"."</font>";			
				}		
			}
			else
			{
				$msg.="<font color='red'>"."此行程編號[comment_info]已重複-ERROR"."</font>";
			}								
		}
	}
	else
	{
		$msg="<font color='red'>"."此評核行程不存在-ERROR"."</font>";
	}								
//--------------------------------------
//disconnect database
//--------------------------------------	
              ...
              <tr> 
                <td background="images/block-top01.gif" height="60" valign="top"> 
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr> 
                      <td width="7%">&nbsp;</td>
                      <td width="93%"><b><font color="#FFFFFF">輔導評核管理-維護輔導評核</font></b></td>
                    </tr>
                  </table>
                </td>
              </tr>
